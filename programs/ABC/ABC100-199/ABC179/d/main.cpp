...
int main()
{
	...
	SegTreeAbstract<mint> segTreeSum(N+1, [](mint x, mint y) { return x + y; }, 0);


	segTreeSum.update(1,1);
	reps(i,2,N+1) {
		mint v = 0;
		rep(k,K) {
			if(1 <= i - L[k]){
				ll l = MAX(i - R[k],1);
				ll r = i- L[k] + 1;
				v += segTreeSum.query(l,r);
			}
		}
		segTreeSum.update(i,v);
	}

	out(segTreeSum.query(N,N+1).val())
