...
int main()
{
	...
	SegTreeAbstract<ll> segTreeString(N+1,  [](ll x, ll y) { return x | y;  }, 0 );

	rep(i,N) {
		ll v = (1 << (S[i] - 'a'));
		segTreeString.update(i+1,v);
	}
	...
	while(Q--) {
		ll t; cin >> t;
		if(t == 1) {
			ll i; string c; cin >> i >> c;
			
			ll v = (1 << (c[0] - 'a'));
			segTreeString.update(i,v);

		} else {
			ll l, r; cin >> l >> r;
			ll v = segTreeString.query(l,r+1);
			ll ans = 0;
			rep(i,26) if( (v & (1 << i) ) > 0) ans ++;
			out(ans)
		}
	}
