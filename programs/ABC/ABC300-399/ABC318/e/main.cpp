...
int main()
{
	...
	SegTreeAbstract<ll> same(n+1, [](ll x, ll y) { return x + y; }, 0);
	vector<ll> a(n);
	rep(i,n) {
		cin >> a[i];
		right[a[i]]++;
	...
	reps(i,1,n-1) {
		...
		ll now1 = left[a[i-1]]*right[a[i-1]];
		ll now2 = left[a[i]]*right[a[i]];

		same.update(a[i-1], now1);
		same.update(a[i],   now2);

		ans += (same.query(1,n+1) - now2);
	}
	out(ans)
