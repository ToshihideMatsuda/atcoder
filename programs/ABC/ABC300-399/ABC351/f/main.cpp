...
int main()
{
	...
	rep(i,N) {
		index[sortA[i].second] = i;
	}

	SegTree<ll> segTreeSum(N+1,[](ll x, ll y) { return x + y; }, 0);
	SegTree<ll> segTreeBit(N+1,[](ll x, ll y) { return x + y; }, 0);

	segTreeSum.update(index[N-1],A[N-1]);
	segTreeBit.update(index[N-1],1);

	ll ans = 0;
	for(int i = N-2;i > -1; i--) {
		ll v   = segTreeSum.query(index[i],N);
		ll cnt = segTreeBit.query(index[i],N);
		ans += v - A[i]*cnt;

		segTreeSum.update(index[i],A[i]);
		segTreeBit.update(index[i],1);
	}
	out(ans)
	return 0;
}