...
**
** 補足: 1山 Nim の場合にはGrundy数 Gが山の石の数Aと一致する
**      N山 Nim の場合にはGrundy数 Gは各山xorの値と一致する（スプレイグ・グランディの定理）ので以下のようになる
**          G = G[1] xor G[2] xor ... G[N] = A[1] xor A[2] xor ... A[N]
**      Grundy数が0の場合には後手必勝、それ以外の場合には先手必勝
*}      （Grundy数が0 <=> 0に遷移することができない。Grundy数が1以上 <=> 0に遷移することが可能。）
*/
